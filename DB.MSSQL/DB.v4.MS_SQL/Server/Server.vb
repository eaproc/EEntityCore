...
Imports EPRO.Library.Objects
Imports EPRO.Library.Types
Imports EPRO.Library
Imports EPRO.Library.Modules



Imports DB.Exceptions
...
#Region "Private"
    ...
    Public Const DEFAULT_DB As String = "master"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Friend CurrentDBInUse As String = DEFAULT_DB

    ''' <summary>
    ''' Tracks Connection
    ''' </summary>
    ''' <remarks></remarks>
    Private ConnectionTracker As Thread = Nothing
    Private MyThreadManager As New v3._5.MicrosoftOS.MultiThreading()
...
#Region "Public "
    ...
    ''' <summary>
    ''' REM If you are Using Tracker .. Remember to set isDBCreated property to true to track your own DB ..else it is master by default.
    ''' Tracks Connection For Easy Read. Remember to Dispose Class
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property KeepTrackingConnection As Boolean

        Get

            If Me.ConnectionTracker Is Nothing Then Return False
            Return Me.ConnectionTracker.IsAlive

        End Get
        Set(ByVal value As Boolean)

            If Not value And Me.KeepTrackingConnection Then
                ''Me.ConnectionTracker.Abort()
                ''Me.ConnectionTracker = Nothing
                Me.MyThreadManager.Abort(Me.ConnectionTracker)


            ElseIf value And Not Me.KeepTrackingConnection Then
                REM If it is true and tracker is not on already
                Me.ConnectionTracker = Me.MyThreadManager.PlaceThreadInInfiniteLoop(
                    AddressOf TrackConnection,
                    2000
                    )

                REM Call the track connection incase anyone needs it immediately
    ...
    Private _ConnectionState As DBConnectionState = DBConnectionState.Unknown

    ''' <summary>
    ''' Note: Only Used when tracker is On ...Else it will always return disconnected
    ''' Returns the Connection state. .. Faster if Tracker is On
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public ReadOnly Property Threaded_ConnectionState As DBConnectionState
        Get

            If Me.KeepTrackingConnection Then Return Me._ConnectionState
            ' ''If Me.CanConnect(True) Then Return DBConnectionState.Connected

            Return DBConnectionState.Disconnected
    ...
    '' ''' <returns></returns>
    '' ''' <remarks></remarks>
    ''Public Overridable ReadOnly Property getConnectionString() As String
    ''    Get
    ''        Dim svrServer As String = Me.SQLServerAddress

    ''        Return "Data Source=" & svrServer & ";Initial Catalog=" & Me.CurrentDBInUse &
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Overridable Overloads Function getConnectionString() As String
    ...
    Public Overloads Function getConnectionString(ByVal DatabaseN As String) As String
        ...
        Return "Data Source=" & svrServer & ";Initial Catalog=" & DatabaseN &
                ";Integrated Security=True;User ID=;Password="
    ...
    ''Private __NoThread_ConnectionState As DBConnectionState = DBConnectionState.Disconnected
    '' ''' <summary>
    ...
    '' ''' <returns></returns>
    '' ''' <remarks></remarks>
    ''Public Overrides ReadOnly Property NoThread_ConnectionState As DBConnectionState
    ''    Get
    ''        Return __NoThread_ConnectionState
    ''    End Get
    ''End Property


    REM Not neccessary to override since its ancesstors already has it
    'Protected Logger As Log1 = New Log1(Me.[GetType]()).Load(Log1.Modes.FILE, True)



    '' ''' <summary>
    '' ''' This allows the tracker to know if it is tracking the UseDefaultDB or Master DB
    '' ''' </summary>
    '' ''' <value></value>
    '' ''' <returns></returns>
    '' ''' <remarks></remarks>
    ''Public Property isDBCreated As Boolean = False
...
#Region "Constants"
    ...
    Public Const COLLATION___Latin1_General_CI_AS As String = "Latin1_General_CI_AS"
...
#Region "Constructors"
    ...
    Sub New(ByVal useDatabase As String)
        Me.New(useDatabase, My.Computer.Name & "\sqlexpress")
    End Sub

    ''' <summary>
    ''' Set Database to use Master
    ''' </summary>
    ''' <remarks></remarks>
    Sub New()
        Me.New(DEFAULT_DB)
...
#Region "Private"
    ...
    ''' <summary>
    ''' If on, tracks every 2secs
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub TrackConnection()

        Dim aCon As SqlConnection = Me.getSQLConnection(CurrentDBInUse)

        ''Dim DBToUse As String = DEFAULT_DB
        ''If Me.isDBCreated Then DBToUse = Me.CurrentDBInUse


        Try

            If aCon Is Nothing Then
                Me._ConnectionState = DBConnectionState.Disconnected
                REM Re-Create Thread but Slower Time Interval
                REM To Avoid Recursive Stack Overflow

            Else
                Me._ConnectionState = DBConnectionState.Connected
        ...
        Finally
            ...
            If aCon IsNot Nothing Then
                If aCon.State <> Data.ConnectionState.Closed Then aCon.Close()
                aCon = Nothing
    ...
    Protected Overridable Sub Dispose(ByVal disposing As Boolean)
        If Not Me.disposedValue Then
            If disposing Then
                ...
                If Me.ConnectionTracker IsNot Nothing Then
                    If Me.ConnectionTracker.IsAlive Then Me.ConnectionTracker.Abort()
                    Me.ConnectionTracker = Nothing
...
#Region "dbExec and getRS"
    ...
    Public Function getSQLConnection() As SqlClient.SqlConnection
        If Me.KeepTrackingConnection And Me.Threaded_ConnectionState <> DBConnectionState.Unknown Then
            If Me.Threaded_ConnectionState = DBConnectionState.Disconnected Then
                Return Nothing
            End If
        End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Overridable Function getSQLConnection(ByVal InitialCatalog As String) As SqlClient.SqlConnection

        Dim sCon As SqlConnection
        'Dim dllFunc As New dllFunctionsNoEvents.clsFuncNoEvents

        '
        '   Fill in Factory Settings
        '

        REM Dim svrServer As String = Me.SQLServerAddress
        ...
        Try
            ...
            REM If no error on open it means it is connected
            'Me.__NoThread_ConnectionState = DBConnectionState.Connected
            Return sCon

        Catch ex As Exception
            'Me.__NoThread_ConnectionState = DBConnectionState.Disconnected
            Logger.Log(New SQLServerConnectionException("Error Connecting to MS SQL Server", Me.getConnectionString(InitialCatalog), ex))
            Return Nothing
        End Try
    ...
    Public Overloads Function dbExec(ByVal StoredProcedure As String,
        ...
        If Me.KeepTrackingConnection Then
            If Me.Threaded_ConnectionState = DBConnectionState.Disconnected Then Return False
        End If

        Dim sqlDBCon As New SqlClient.SqlConnection
        sqlDBCon = getSQLConnection()
        If IsNothing(sqlDBCon) Then Return False
    ...
    Public Overloads Function dbExec(ByVal SQL As String,
        ...
        '
        If Me.KeepTrackingConnection AndAlso Me.DatabaseName.equalsIgnoreCase(UsingDatabase) Then
            If Me.Threaded_ConnectionState = DBConnectionState.Disconnected Then Return False
        End If
    ...
    Public Overloads Function getRS(ByVal SQL As String,
        ...
        '
        If Me.KeepTrackingConnection AndAlso Me.DatabaseName.equalsIgnoreCase(UsingDatabase) Then
            If Me.Threaded_ConnectionState = DBConnectionState.Disconnected Then Return Nothing
        End If
        ...
        Dim sqlDBCon As New SqlClient.SqlConnection
        sqlDBCon = getSQLConnection(UsingDatabase)
        If IsNothing(sqlDBCon) Then Return Nothing
...
#Region "Testing Connection"
    ...
    Public Overrides Function PingDB() As Boolean
        ...
        Return IsDataSetValid(
            getRS("SELECT * FROM sys.server_principals", DEFAULT_DB)
            )
    ...
    Public Overrides Function CanConnect() As Boolean
        ...
        Dim aWait As New WaitAsync(, "Connecting to Database with the supplied parameters ...")
        Dim aCon As SqlConnection = Me.getSQLConnection()

        Try
            If Not IsNothing(aCon) Then Return True
        Catch ex As Exception
            Logger.Log(New EException("Exception Under CanConnect (SQL Server)", ex))
        Finally

            If Not IsNothing(aCon) Then
    ...
    Public Overloads Function CanConnect(ByVal Silent As Boolean) As Boolean
        ...
        If Not Silent Then Return Me.CanConnect
...
#Region "Saving Image to Database"
    ...
    Public Overloads Function SaveThisImageOnTable(ByVal TableName As String,
                                         ...
                                         Optional ByVal ImageToSave As Image = Nothing) As Boolean
    ...
    Public Overloads Function SaveThisImageOnTable(ByVal TableName As String,
                                         ByVal ImageColumnName As String,
                                          ByVal ColumnIDName As String,
                                         ByVal ColumnID As Long,
                                         ByVal ImageFormatToUse As Imaging.ImageFormat,
                                         Optional ByVal ImageToSave As Image = Nothing) As Boolean
        ...
        Try
            ...
            If ImageToSave IsNot Nothing Then
                ...
                Dim cmd As New SqlClient.SqlCommand(sql, Con)
                Dim ms As New IO.MemoryStream()
                ImageToSave.Save(ms, ImageFormatToUse)
                data = ms.GetBuffer()
                Dim p As New SqlClient.SqlParameter("@photo", SqlDbType.Image)
                p.Value = data
                cmd.Parameters.Add(p)
    ...
    Public Function FetchImageFromTable(ByVal TableName As String,
                                         ByVal ImageColumnName As String,
                                         ByVal ColumnIDName As String,
                                         ByVal ColumnID As Long) As Image
    ...
    Public Overloads Function ExecuteSQLFile(ByVal SQLFileName As String,
        ...
        Try
            ...
            If FileIO.FileSystem.FileExists(SQLFileName) Then
                ...
                Dim aWait As New WaitAsync(, "Executing SQL Scripts ...")
                ...
                For Each SQLCommand As String In SQLs
                    ...
                    aWait.setProgressBar(SQLs.Count, CLng(iCount))
...
#Region "Saving Date and Time in SQL Server 2008"
    ...
    Public Overrides Function GetSQLDateTimeFormat(ByVal _DateTime As NullableDateTime) As String

        Return Me.GetSQLDateTimeFormat(_DateTime, False)

    End Function

    Public Overloads Function GetSQLDateTimeFormat(ByVal _DateTime As NullableDateTime,
    ...
    Public Overloads Function GetSQLDateFormat(ByVal _Date As NullableDateTime,
                                                            ByVal EncodeApostrophe As Boolean) As String
    ...
    Public Overrides Function GetSQLDateFormat(ByVal _Date As NullableDateTime) As String

        Return Me.GetSQLDateFormat(_Date, False)

    End Function

    Public Overloads Function GetSQLTimeFormat(ByVal _Time As NullableDateTime,
                                                            ByVal EncodeApostrophe As Boolean) As String
    ...
    Public Overrides Function GetSQLTimeFormat(ByVal _Time As NullableDateTime) As String
