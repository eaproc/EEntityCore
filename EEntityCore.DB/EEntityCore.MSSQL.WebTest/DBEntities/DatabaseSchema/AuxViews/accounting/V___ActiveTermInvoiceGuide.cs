...
{                  
 ...
 #region Methods                                                      
        ...
        public static V___ActiveTermInvoiceGuide GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new V___ActiveTermInvoiceGuide(conn.Fetch(ActiveTermInvoiceGuide__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static V___ActiveTermInvoiceGuide GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new V___ActiveTermInvoiceGuide( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
