...
{                  
 ...
 #region Methods                                                      
        ...
        public static V___TermView GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new V___TermView(conn.Fetch(TermView__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static V___TermView GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new V___TermView( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
