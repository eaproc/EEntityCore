...
{                  
 ...
 #region Methods                                                      
        ...
        public static V___MonthlyDonationView GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new V___MonthlyDonationView(conn.Fetch(MonthlyDonationView__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static V___MonthlyDonationView GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new V___MonthlyDonationView( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
