...
{                  
 ...
 #region Methods                                                      
        ...
        public static V___CenterView GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new V___CenterView(conn.Fetch(CenterView__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static V___CenterView GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new V___CenterView( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
