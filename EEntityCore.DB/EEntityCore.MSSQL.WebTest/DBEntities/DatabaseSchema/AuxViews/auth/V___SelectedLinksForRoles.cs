...
{                  
 ...
 #region Methods                                                      
        ...
        public static V___SelectedLinksForRoles GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new V___SelectedLinksForRoles(conn.Fetch(SelectedLinksForRoles__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static V___SelectedLinksForRoles GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new V___SelectedLinksForRoles( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
