...
{                  
 ...
 #region Methods                                                      
        ...
        public static V___UserVerifiedContact GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new V___UserVerifiedContact(conn.Fetch(UserVerifiedContact__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static V___UserVerifiedContact GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new V___UserVerifiedContact( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
