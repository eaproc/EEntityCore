...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___PasswordResetHistory GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___PasswordResetHistory(conn.Fetch(PasswordResetHistory__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___PasswordResetHistory GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___PasswordResetHistory( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
