...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Session GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Session(conn.Fetch(Session__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Session GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Session( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
