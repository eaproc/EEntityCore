...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Role GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Role(conn.Fetch(Role__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Role GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Role( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
