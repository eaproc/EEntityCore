...
{                  
 ...
 #region Constructors                  
       ...
       static T___LinkPermissionMember()                  
        {                  
          ...
          defHasPermissionID = new DataColumnDefinition(TableColumnNames.HasPermissionID.ToString(), typeof(int),false, DataColumnDefinition.ConstraintTypes.FOREIGN);
   ...
   #region Partial Access                                    
        ...
        /// <param name="TargettedRowID"></param>                                                      
        /// <remarks></remarks>                                    
        public T___LinkPermissionMember(DataTable FullTable) : base(FullTable)                                    
        {                                    
        }                                    
 ...
 #region Consts and Enums                       
       ...
       public const string LinkPermissionMember__NO__BINARY___SQL_FILL_QUERY = "SELECT [ID], [LinkPermissionID], [HasPermissionID], [CreatedAt] FROM auth.LinkPermissionMember";
       public const string LinkPermissionMember__ALL_COLUMNS___SQL_FILL_QUERY = "SELECT [ID], [LinkPermissionID], [HasPermissionID], [CreatedAt] FROM auth.LinkPermissionMember";
 ...
 #region Methods                                                      
        ...
        public static T___LinkPermissionMember GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___LinkPermissionMember(conn.Fetch(LinkPermissionMember__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___LinkPermissionMember GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___LinkPermissionMember( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
        ...
        {                  
            ...
            private DataColumnParameter ParamHasPermissionID;
            ...
            public UpdateQueryBuilder SetHasPermissionID(int v)                  
            {                  
                ParamHasPermissionID = new(defHasPermissionID, v);                  
                return this;                  
            }                  
...
,            DateTime CreatedAt
            ...
            {                   
                    ...
                    string.Format(" INSERT INTO {0}([LinkPermissionID],[HasPermissionID],[CreatedAt]) VALUES({1},{2},{3})  ", TABLE_NAME,
                        paramLinkPermissionID.GetSQLValue(),
                        paramHasPermissionID.GetSQLValue(),
