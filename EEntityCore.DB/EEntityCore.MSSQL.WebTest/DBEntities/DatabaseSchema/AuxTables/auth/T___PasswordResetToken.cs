...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___PasswordResetToken GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___PasswordResetToken(conn.Fetch(PasswordResetToken__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___PasswordResetToken GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___PasswordResetToken( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
