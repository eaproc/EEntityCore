...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Users GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Users(conn.Fetch(Users__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Users GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Users( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
