...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Link GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Link(conn.Fetch(Link__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Link GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Link( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
