...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Bank GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Bank(conn.Fetch(Bank__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Bank GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Bank( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
