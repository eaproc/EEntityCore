...
{                  
 ...
 #region Constructors                  
       ...
       static T___Relation()                  
        {                  
          ...
          defRelatedToID = new DataColumnDefinition(TableColumnNames.RelatedToID.ToString(), typeof(int),false, DataColumnDefinition.ConstraintTypes.UNIQUE);
          defRelationshipTypeID = new DataColumnDefinition(TableColumnNames.RelationshipTypeID.ToString(), typeof(int),false, DataColumnDefinition.ConstraintTypes.FOREIGN);
   ...
   #region Partial Access                                    
        ...
        /// <remarks></remarks>                                    
        public T___Relation(DataTable FullTable, long TargettedRowID) : base(FullTable, TargettedRowID)                                    
        {                                    
        }                                    
 ...
 #region Consts and Enums                       
       ...
       public const string Relation__NO__BINARY___SQL_FILL_QUERY = "SELECT [ID], [PersonID], [RelatedToID], [RelationshipTypeID], [CreatedAt] FROM common.Relation";
       public const string Relation__ALL_COLUMNS___SQL_FILL_QUERY = "SELECT [ID], [PersonID], [RelatedToID], [RelationshipTypeID], [CreatedAt] FROM common.Relation";
 ...
 #region Properties 
       ...
       public int RelatedToID { get => (int)TargettedRow.GetDBValueConverted<int>(TableColumnNames.RelatedToID.ToString());  set => TargettedRow[TableColumnNames.RelatedToID.ToString()] = value; }
 ...
 #region Methods                                                      
        ...
        public static T___Relation GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Relation(conn.Fetch(Relation__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Relation GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Relation( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
                                                                        
        public T___Relation GetRowWhereID(long pID) => new(this.RawTable, pID);                                                      
        ...
        {                  
            ...
            private DataColumnParameter ParamRelatedToID;
            ...
            public UpdateQueryBuilder( T___Relation v):this(v.ID)                  
            {                  

                ParamPersonID = new(defPersonID, v.PersonID);                  
                ParamRelatedToID = new(defRelatedToID, v.RelatedToID);                  
            ...
            public UpdateQueryBuilder SetRelatedToID(int v)                  
            {                  
                ParamRelatedToID = new(defRelatedToID, v);                  
                return this;                  
            }                  
...
,            DateTime CreatedAt
            ...
            return r.Run( (conn) =>                   
                      conn.ExecuteTransactionQuery(                  
                    string.Format(" SET IDENTITY_INSERT {0} ON INSERT INTO {0}([ID],[PersonID],[RelatedToID],[RelationshipTypeID],[CreatedAt]) VALUES({1},{2},{3},{4},{5})  SET IDENTITY_INSERT {0} OFF ", TABLE_NAME,
                        paramID.GetSQLValue(),
                        paramPersonID.GetSQLValue(),
                        paramRelatedToID.GetSQLValue(),
                        paramRelationshipTypeID.GetSQLValue(),
