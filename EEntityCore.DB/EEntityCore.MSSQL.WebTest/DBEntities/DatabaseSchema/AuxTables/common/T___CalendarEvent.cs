...
#region Constructors                  
       ...
       public T___CalendarEvent() : base()                  
        ...
        /// <remarks></remarks>                                    
        public T___CalendarEvent(DataTable FullTable, int TargettedRowID) : base(FullTable, TargettedRowID)                                    
        {                                    
        }                                    
...
#region Methods                  
        ...
        {                  
            if (this.HasRows())                  
                return new T___CalendarEvent(this.AllRows.First());                  
            return null;                  
        }                  
        ...
        {                  
            return new T___CalendarEvent(DBConnectInterface.GetDBConn(), string.Format("SELECT * FROM {0} WHERE ID={1}", (object)pID)).GetFirstRow();                  
        }                  
                  
        public T___CalendarEvent GetRowWhereID(int pID)                  
        {                  
            return new T___CalendarEvent(this.RawTable, pID);                  
        }                  
        ...
        {                  
            ...
            {                  
                ...
                if (drst is object && drst.Count() > 0)                  
                    return new T___CalendarEvent(drst);                  
                return null;                  
            }                  
            catch (Exception)                  
            {                  
...
DateTime pCreatedAt){
            ...
            try{
                ...
                DataColumnParameter paramID = new DataColumnParameter(defID, DatabaseInit.DBConnectInterface.GetDBConn().GETNewID(TABLE_NAME));
                DataColumnParameter paramDefinition = new DataColumnParameter(defDefinition, pDefinition);
                DataColumnParameter paramClassColor = new DataColumnParameter(defClassColor, pClassColor);
