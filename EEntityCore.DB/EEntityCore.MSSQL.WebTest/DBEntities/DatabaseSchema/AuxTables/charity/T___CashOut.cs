...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___CashOut GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___CashOut(conn.Fetch(CashOut__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___CashOut GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___CashOut( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
