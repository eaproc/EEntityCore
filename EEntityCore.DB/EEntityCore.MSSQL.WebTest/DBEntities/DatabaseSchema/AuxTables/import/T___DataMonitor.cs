...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___DataMonitor GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___DataMonitor(conn.Fetch(DataMonitor__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___DataMonitor GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___DataMonitor( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
