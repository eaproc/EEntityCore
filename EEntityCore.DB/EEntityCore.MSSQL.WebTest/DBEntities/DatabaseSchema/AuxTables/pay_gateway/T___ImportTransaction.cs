...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___ImportTransaction GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___ImportTransaction(conn.Fetch(ImportTransaction__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___ImportTransaction GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___ImportTransaction( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
