...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___PaymentTransaction GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___PaymentTransaction(conn.Fetch(PaymentTransaction__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___PaymentTransaction GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___PaymentTransaction( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
