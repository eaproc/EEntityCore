...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___PayoutTransaction GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___PayoutTransaction(conn.Fetch(PayoutTransaction__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___PayoutTransaction GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___PayoutTransaction( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
