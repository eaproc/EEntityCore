...
{                  
 ...
 #region Constructors                  
       ...
       static T___ImportTransactionFile()                  
        {                  
          ...
          defFilePath = new DataColumnDefinition(TableColumnNames.FilePath.ToString(), typeof(string),false, DataColumnDefinition.ConstraintTypes.UNKNOWN);
   ...
   #region Partial Access                                    
        ...
        /// <remarks></remarks>                                    
        public T___ImportTransactionFile(DataTable FullTable, long TargettedRowID) : base(FullTable, TargettedRowID)                                    
        {                                    
        }                                    
        ...
        /// <param name="TargettedRowID"></param>                                                      
        /// <remarks></remarks>                                    
        public T___ImportTransactionFile(DataTable FullTable) : base(FullTable)                                    
        {                                    
        }                                    
 ...
 #region Consts and Enums                       
       ...
       public const string ImportTransactionFile__NO__BINARY___SQL_FILL_QUERY = "SELECT [ID], [FilePath], [CreatedAt], [CreatedByID] FROM pay_gateway.ImportTransactionFile";
       public const string ImportTransactionFile__ALL_COLUMNS___SQL_FILL_QUERY = "SELECT [ID], [FilePath], [CreatedAt], [CreatedByID] FROM pay_gateway.ImportTransactionFile";
 ...
 #region Properties 
       ...
       public string FilePath { get => (string)TargettedRow.GetDBValueConverted<string>(TableColumnNames.FilePath.ToString());  set => TargettedRow[TableColumnNames.FilePath.ToString()] = value; }
 ...
 #region Methods                                                      
        ...
        public static T___ImportTransactionFile GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___ImportTransactionFile(conn.Fetch(ImportTransactionFile__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___ImportTransactionFile GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___ImportTransactionFile( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
                                                                        
        public T___ImportTransactionFile GetRowWhereID(long pID) => new(this.RawTable, pID);                                                      
        ...
        {                  
            ...
            private DataColumnParameter ParamFilePath;
            ...
            public UpdateQueryBuilder( T___ImportTransactionFile v):this(v.ID)                  
            {                  

                ParamFilePath = new(defFilePath, v.FilePath);                  
                ParamCreatedAt = new(defCreatedAt, v.CreatedAt);                  
                ParamCreatedByID = new(defCreatedByID, v.CreatedByID);                  
            }                  
                  
            public UpdateQueryBuilder SetFilePath(string v)                  
            {                  
                ParamFilePath = new(defFilePath, v);                  
                return this;                  
            }                  
...
,            int CreatedByID
                ...
                DataColumnParameter paramFilePath = new (defFilePath, FilePath);
            ...
            {                   
                    ...
                    string.Format(" INSERT INTO {0}([FilePath],[CreatedAt],[CreatedByID]) VALUES({1},{2},{3})  ", TABLE_NAME,
                        paramFilePath.GetSQLValue(),
...
,            int CreatedByID
            ...
            return r.Run( (conn) =>                   
                      conn.ExecuteTransactionQuery(                  
                    string.Format(" SET IDENTITY_INSERT {0} ON INSERT INTO {0}([ID],[FilePath],[CreatedAt],[CreatedByID]) VALUES({1},{2},{3},{4})  SET IDENTITY_INSERT {0} OFF ", TABLE_NAME,
                        paramID.GetSQLValue(),
                        paramFilePath.GetSQLValue(),
                        paramCreatedAt.GetSQLValue(),
                        paramCreatedByID.GetSQLValue()                        )
