...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Client GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Client(conn.Fetch(Client__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Client GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Client( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
