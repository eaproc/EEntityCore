...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Payroll GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Payroll(conn.Fetch(Payroll__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Payroll GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Payroll( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
