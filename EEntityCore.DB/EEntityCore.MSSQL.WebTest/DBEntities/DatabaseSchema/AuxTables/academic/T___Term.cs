...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Term GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Term(conn.Fetch(Term__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Term GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Term( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
