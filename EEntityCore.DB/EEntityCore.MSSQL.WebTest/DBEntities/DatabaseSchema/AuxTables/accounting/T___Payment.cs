...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Payment GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Payment(conn.Fetch(Payment__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Payment GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Payment( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
