...
{                  
 ...
 #region Methods                                                      
        ...
        public static T___Dealer GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___Dealer(conn.Fetch(Dealer__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___Dealer GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___Dealer( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
