...
{                  
 ...
 #region Constructors                  
       ...
       static T___ReferralPayment()                  
        {                  
          ...
          defReferredClientID = new DataColumnDefinition(TableColumnNames.ReferredClientID.ToString(), typeof(int),false, DataColumnDefinition.ConstraintTypes.FOREIGN);
 ...
 #region Consts and Enums                       
       ...
       public const string ReferralPayment__NO__BINARY___SQL_FILL_QUERY = "SELECT [ID], [ReferredClientID], [Amount], [TermID], [Comments], [CreatedByID], [CreatedAt] FROM accounting.ReferralPayment";
       public const string ReferralPayment__ALL_COLUMNS___SQL_FILL_QUERY = "SELECT [ID], [ReferredClientID], [Amount], [TermID], [Comments], [CreatedByID], [CreatedAt] FROM accounting.ReferralPayment";
 ...
 #region Methods                                                      
        ...
        public static T___ReferralPayment GetFullTable(TransactionRunner runner) =>                   
            runner.Run( (conn) =>                  
                new T___ReferralPayment(conn.Fetch(ReferralPayment__ALL_COLUMNS___SQL_FILL_QUERY).FirstTable())                  
                );                                                      
                                                      
        public static T___ReferralPayment GetRowWhereIDUsingSQL(long pID, TransactionRunner runner)                                                                        
        {                  
            return runner.Run( (conn) =>  new T___ReferralPayment( conn.Fetch($"SELECT * FROM {TABLE_NAME} WHERE ID={pID}" ).FirstTable(), pID )                  
                );                  
        }                                                                        
        ...
        {                  
            ...
            private DataColumnParameter ParamReferredClientID;
            ...
            {                  
                ParamReferredClientID = new(defReferredClientID, v);                  
                return this;                  
            }                  
