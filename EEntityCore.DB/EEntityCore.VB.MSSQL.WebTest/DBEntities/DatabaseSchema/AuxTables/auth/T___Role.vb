...
ColumnDefns = New Dictionary(Of String, DataColumnDefinition)()
  ...
  defRank = New DataColumnDefinition(TableColumnNames.Rank.ToString(), GetType(Byte), False, Nothing,DataColumnDefinition.ConstraintTypes.UNKNOWN) 
...
#Region "Consts and Enums"
       ...
       Public Const TABLE_NAME As String = "auth.Role"
       Public Const Role__NO__BINARY___SQL_FILL_QUERY As String = "SELECT [ID], [Name], [Description], [CanBeUpdated], [CanBeDeleted], [CreatedAt], [UpdatedAt], [Rank] FROM Role"
       Public Const Role__ALL_COLUMNS___SQL_FILL_QUERY As String = "SELECT [ID], [Name], [Description], [CanBeUpdated], [CanBeDeleted], [CreatedAt], [UpdatedAt], [Rank] FROM Role"
...
#Region "Methods"                  
        ...
        ''' <returns></returns>                                     
        ''' <remarks></remarks>                                     
        Public Function getFirstRow() As T___Role                  
            If Me.hasRows() Then Return New T___Role(Me.AllRows(0))                  
            Return New T___Role()                  
        End Function                  
        ...
        Public Function getRowWhereID(pID As Int32) As T___Role                  
            Return New T___Role(Me.RawTable, T___Role.TABLE_NAME, pID)                  
        End Function                  
                  
                  
        Private Function IsAllRowEqual(ByVal pRow As T___Role,                  
        ...
        Private Function IsAnyRowEqual(ByVal pRow As T___Role,                  
                                            ParamArray pParams() As DataColumnParameter                  
                                            ) As Boolean                  
            Try                  
                If Not Me.hasRows() Then Return False                  
        ...
        ''' <returns></returns>                                     
        ''' <remarks></remarks>                                     
        Public Function getAllRowsEquals(ParamArray pParams() As DataColumnParameter) As T___Role                  
            ...
            Try                  
                ...
                Where Me.IsAllRowEqual(New T___Role(dr), pParams)                  
                Select dr                  
                  
                  
                If drst IsNot Nothing AndAlso drst.Count() > 0 Then Return New T___Role(drst.CopyToDataTable(),                  
                    T___Role.TABLE_NAME)                  
                Return New T___Role()                  
            Catch ex As Exception                  
                Logger.Print(ex)                  
        ...
        ''' <returns></returns>                                     
        ''' <remarks></remarks>                                     
        Public Function getAllRowsAny(ParamArray pParams() As DataColumnParameter) As T___Role                  
            If pParams Is Nothing OrElse pParams.Count = 0 OrElse Not Me.hasRows() Then Return Me                  
            Try                  
                  
                Dim drst As IEnumerable(Of DataRow) = From dr As DataRow In Me.AllRows                  
                Where Me.IsAnyRowEqual(New T___Role(dr), pParams)                  
                Select dr                  
...
Optional ByVal pRank As Object=Nothing,
