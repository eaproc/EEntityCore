...
Public Class T___CalendarEvent
...
#Region "Consts and Enums"
       ...
       Public Const TABLE_NAME As String = "common.CalendarEvent"
       Public Const CalendarEvent__NO__BINARY___SQL_FILL_QUERY As String = "SELECT [ID], [Definition], [ClassColor], [CanBeDeleted], [CreatedAt] FROM CalendarEvent"
       Public Const CalendarEvent__ALL_COLUMNS___SQL_FILL_QUERY As String = "SELECT [ID], [Definition], [ClassColor], [CanBeDeleted], [CreatedAt] FROM CalendarEvent"
...
#Region "Methods"                  
        ...
        ''' <returns></returns>                                     
        ''' <remarks></remarks>                                     
        Public Function getFirstRow() As T___CalendarEvent                  
            If Me.hasRows() Then Return New T___CalendarEvent(Me.AllRows(0))                  
            Return New T___CalendarEvent()                  
        End Function                  
                  
                  
        Public Shared Function getFullTable() As T___CalendarEvent                  
            Return New T___CalendarEvent(DBConnectInterface.GetDBConn(), TABLE_NAME)                  
        End Function                  
                  
        Public Shared Function getRowWhereIDUsingSQL(ByVal pID As Int32) As T___CalendarEvent                  
            Return New T___CalendarEvent(DBConnectInterface.GetDBConn(),                  
                                               TABLE_NAME,                  
                                               String.Format("SELECT * FROM {0} WHERE ID={1}", TABLE_NAME, pID)                  
                                               ).getFirstRow()                  
        End Function                  
                  
        Public Function getRowWhereID(pID As Int32) As T___CalendarEvent                  
            Return New T___CalendarEvent(Me.RawTable, T___CalendarEvent.TABLE_NAME, pID)                  
        End Function                  
                  
                  
        Private Function IsAllRowEqual(ByVal pRow As T___CalendarEvent,                  
        ...
        Private Function IsAnyRowEqual(ByVal pRow As T___CalendarEvent,                  
                                            ParamArray pParams() As DataColumnParameter                  
                                            ) As Boolean                  
            Try                  
                If Not Me.hasRows() Then Return False                  
        ...
        ''' <returns></returns>                                     
        ''' <remarks></remarks>                                     
        Public Function getAllRowsEquals(ParamArray pParams() As DataColumnParameter) As T___CalendarEvent                  
            ...
            Try                  
                ...
                Where Me.IsAllRowEqual(New T___CalendarEvent(dr), pParams)                  
                Select dr                  
                  
                  
                If drst IsNot Nothing AndAlso drst.Count() > 0 Then Return New T___CalendarEvent(drst.CopyToDataTable(),                  
                    T___CalendarEvent.TABLE_NAME)                  
                Return New T___CalendarEvent()                  
            Catch ex As Exception                  
                Logger.Print(ex)                  
        ...
        ''' <returns></returns>                                     
        ''' <remarks></remarks>                                     
        Public Function getAllRowsAny(ParamArray pParams() As DataColumnParameter) As T___CalendarEvent                  
            If pParams Is Nothing OrElse pParams.Count = 0 OrElse Not Me.hasRows() Then Return Me                  
            Try                  
                  
                Dim drst As IEnumerable(Of DataRow) = From dr As DataRow In Me.AllRows                  
                Where Me.IsAnyRowEqual(New T___CalendarEvent(dr), pParams)                  
                Select dr                  
...
Try
 ...
 Dim paramClassColor As DataColumnParameter = New DataColumnParameter(defClassColor, defClassColor.DefaultValue)
...
Try
 ...
 Dim paramID As DataColumnParameter = New DataColumnParameter(defID, DatabaseInit.DBConnectInterface.GetDBConn().GETNewID(TABLE_NAME))
 Dim paramDefinition As DataColumnParameter = New DataColumnParameter(defDefinition, pDefinition)
 Dim paramClassColor As DataColumnParameter = New DataColumnParameter(defClassColor, pClassColor)
...
Return DBConnectInterface.GetDBConn().dbExec(
     String.Format("INSERT INTO {0}([Definition],[ClassColor],[CanBeDeleted],[CreatedAt]) VALUES({1},{2},{3},{4}) ", TABLE_NAME,paramDefinition.getSQLQuotedValueForAdd(),
paramClassColor.getSQLQuotedValueForAdd(),
paramCanBeDeleted.getSQLQuotedValueForAdd(),
paramCreatedAt.getSQLQuotedValueForAdd()  ), True)
...
Optional ByVal pClassColor As Object=Nothing,
...
DBConnectInterface.GetDBConn().dbExec(
     String.Format("UPDATE {0} SET [Definition]={2},[ClassColor]={3},[CanBeDeleted]={4},[CreatedAt]={5} WHERE ID={1} ", TABLE_NAME, paramID.getSQLQuotedValueForUpdate(),paramDefinition.getSQLQuotedValueForUpdate(),
paramClassColor.getSQLQuotedValueForUpdate(),
